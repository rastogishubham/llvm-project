...
DIExpression *DIExpression::appendOpsToArg(const DIExpression *Expr,
  ...
  if (!any_of(Expr->expr_ops(),
    ...
    if (!SkipFolding) {
      ...
      // Instead of applying a foldConstantMath on the entire expression, apply
      // it only on operations that are preppended + the succeeding operations
      ...
      // should be applied upon, which is the number of new operations added +
      // the succeeding operations totalling to the longest foldable pattern.
